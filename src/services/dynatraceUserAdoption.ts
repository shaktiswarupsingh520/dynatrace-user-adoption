...
export async function fetchLoginEvents(days: 7 | 15 | 30): Promise<LoginEvent[]> {
  const result = await queryExecutionClient.queryExecute({
    body: { query: queryFor(days), requestTimeoutMilliseconds: 30000 },
  });
  const records = (result.result?.records ?? []) as Record<string, unknown>[];
  return records.map((r) => ({
    timestamp: str(r.timestamp),
    userId: str(r['user.id']) || 'Unknown',
    userName: str(r['user.email']) || str(r['user.name']) || undefined,
    provider: str(r['event.provider']) || undefined,
  }));
}
...
export function buildUserActivity(events: LoginEvent[]): UserActivity[] {
  ...
  return [...grouped.entries()].map(([userId, items]) => {
    ...
    return {
      userId,
      userName: ordered.find((x) => x.userName)?.userName ?? userId,
      lastLogin: ordered[0]?.timestamp ?? '',
      activeDays: new Set(ordered.map((x) => x.timestamp.slice(0, 10))).size,
      logins: ordered.length,
      status: ordered.length ? 'Active' : 'Inactive',
      managementZone: 'Pending enrichment',
    };
...
export function dailyActiveUsers(events: LoginEvent[]) {
  ...
  for (const event of events) {
    ...
    const users = daily.get(day) ?? new Set<string>();
